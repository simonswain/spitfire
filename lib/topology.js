...
var Topology = module.exports = function(opts){
  ...
  if(!opts.redis.hasOwnProperty('prefix')){
    ...
    opts.redis.prefix = ':' + opts.redis.prefix;
  }

  var keys = {
    queue: opts.redis.prefix + 'queue'
  };
  ...
  this.remove = function(id, done){
    ...
    this.nodes[id].stop(function(){
      _.each(self.sources[id], function(source_id){
        self.targets[source_id] = _.without(self.targets[source_id], id);
      });
  ...
  // Get value of a node (result of last message output)
